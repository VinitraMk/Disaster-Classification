...
#custom imports
from helper.utils import get_config, get_model_params, get_preproc_params
...
class Preprocessor:
    ...
    def get_tokens_for_dataset(self, data_iter):
        all_tokens = []
        for i, (text, _, _) in enumerate(data_iter):
            #print(self.tokenizer(self.__clean_data(data_iter[i][text])))
            all_tokens.append(self.tokenizer(self.__clean_data(data_iter[i][text])))
        return all_tokens
    ...
    def __build_text_pipeline(self):
        ...
        self.label_pipeline = lambda x: x
