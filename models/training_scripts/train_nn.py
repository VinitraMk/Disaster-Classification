...
class ANN(nn.ModuleList):
    ...
    def in_features_fc(self):
        ...
        #print('no of input features in fc layer', (out_pool_1 + out_pool_2 + out_pool_3 + out_pool_4) * self.out_size)
...
class ExperimentTrain:
    ...
    def __load_data_and_model(self):
        ...
        if self.is_first_batch:
            model_input_path = f"{self.mounted_input_path}/models/ann_model.tar"
        else:
            model_input_path = f"{self.mounted_output_path}/internal_output/ann_model.tar"
        ...
        self.model = ANN(self.vocab_size, self.model_args["embed_dim"], 2, model_args["out_size"], model_args["stride"], model_args["text_max_length"], model_args["final_output_size"])
        ...
        self.optimizer = torch.optim.RMSprop(self.model.parameters(), lr = self.model_args["lr"])
    ...
    def __train_batch(self):
        ...
        if not(self.is_last_batch):
            model_output_path = f"{self.mounted_output_path}/internal_output/ann_model.tar"
        else:
            model_output_path = f"{self.mounted_output_path}/models/ann_model.tar"
